...
export const api = {
  ...
  getPublicAppConfig: () => request<{ web_build?: string; mobile_web_gate?: boolean }>("/public/app-config"),
  adminGetMobileWebGate: () => request<{ mobile_web_gate: boolean }>("/admin/mobile-web-gate"),
  adminSetMobileWebGate: (enabled: boolean) => request<{ mobile_web_gate: boolean }>("/admin/mobile-web-gate", { method: "POST", body: JSON.stringify({ enabled }) }),
